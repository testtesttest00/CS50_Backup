...
bool printrank(void){
    ...
    while (fgets(&line[0], LENGTH+AUX, ranklist)){
        ...
        loadhash(moves,name);
        printf("%5i  %s     loaded to hash table\n", moves, name);
    }
    //test: load & free hash table::success
    ...
    // strcpy(n4->name, "n4");
    // n4->next = n1;
    // node *merge = mergesort_merge(n4, n3);
    // while (merge != NULL){
    //     printf("%p <@ %5i / %s p> %p\n", merge, merge->moves, merge->name, merge->next);
    //     merge = merge->next;
    // }
...
node *mergesort_split(node *linkedlist, int nodecount){
    if (nodecount == 0){
        node *n = linkedlist;
        while(n != NULL){
            n = n->next;
