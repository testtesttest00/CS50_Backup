...
node *stitchhash(int a, int b){
    ...
    node *first = mergesort(hashtable[a]);
    node *second = mergesort(hashtable[b]);
    node *loop = first;
...
bool printtop(int places){ //ternary at hash function lol (forced it in kinda)
    node *list = hashtable[0];
    node *loop = list;
    if (!loop){
    ...
    int records = 1;
    int hashtable_iter = 1;
    do{
        list = stitchhash(0, hashtable_iter);
        hashtable_iter++;
        while (loop->next != NULL){
            loop = loop->next;
            records++;
        }
    } while (records < places && hashtable_iter < ARBITRARY);
    if (hashtable_iter >= ARBITRARY && records < places){
        printf("\nInsufficient records\n");
        return false;
    }
