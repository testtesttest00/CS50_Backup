...
class Puzzle15():
    ...
    def __str__(self):
        return f"\
{str(self.board[0][0]).ljust(3)} {str(self.board[0][1]).ljust(3)} {str(self.board[0][2]).ljust(3)} {str(self.board[0][3]).ljust(3)}\n\
{str(self.board[1][0]).ljust(3)} {str(self.board[1][1]).ljust(3)} {str(self.board[1][2]).ljust(3)} {str(self.board[1][3]).ljust(3)}\n\
{str(self.board[2][0]).ljust(3)} {str(self.board[2][1]).ljust(3)} {str(self.board[2][2]).ljust(3)} {str(self.board[2][3]).ljust(3)}\n\
{str(self.board[3][0]).ljust(3)} {str(self.board[3][1]).ljust(3)} {str(self.board[3][2]).ljust(3)} {str(self.board[3][3]).ljust(3)}"
    ...
    def scrambleboard(self):
        if self.cheats:
            return
        for _ in range(random.randint(0, 500)):
            self.moveboard(random.choice(["u","d","l","r"]))
        return

    def nextmove(self):
        self.moves += 1
        while 1:
            print(f"Turn {self.moves}\n")
            print(self, end = "\n\n")
            move = input("Enter Up (U), Down (D), Left (L) or Right (R)\n\n").strip().lower()
            print(f"\n==-==-==-=-==-==-==-==-==-==-==-==-==-==-==-==\n")
            try:
                if self.moveboard(move[0]):
                    break
            except:
                pass
        self.save_board()
    ...
    def moveboard(self, direction):
        ...
        if x + directions[direction][0] not in range(4) or y + directions[direction][1] not in range(4):
            return False
        self.board[x][y] = self.board[x+directions[direction][0]][y+directions[direction][1]]
        self.board[x+directions[direction][0]][y+directions[direction][1]] = "."
        return True
    ...
    @board.setter
    def board(self, board):
        if len(board) == 4 and isinstance(board, list):
            for row in board:
                for value in row:
...
Player 1: {self.c4p1score} win(s) / {self.c4games} round(s)\n\
Player 2: {self.c4p2score} win(s) / {self.c4games} round(s)\n\n"
        else:
            c4vs = ""
        if self.p15games > 0:
            avg = self.p15score / self.p15games
            p15 = f"Puzzle 15 Score:\n\
Total {self.p15score} move(s) / {self.p15games} round(s)\n\
Average {avg:0.2f} move(s)\n\n"
        else:
            p15 = ""
        return f"\n==-==-==-=-==-==-==-==-==-==-==-==-==-==-==-==\n\n\
{tttcpu}\
    ...
    def input(self, results):
        if inp := re.match(r"1, (\d), (.+)", results):
            ...
            if winner == "Player 1":
                ...
                self.c4p2score += 1
        elif inp := re.match(r"4, (.+)", results):
            self.p15games += 1
            self.p15score += int(inp.group(1))
...
4) Puzzle 15 (Singleplayer)\n\n").lower().strip()
            ...
            elif start in ["4", "p15", "15p", "puzzle 15", "15 puzzle"]:
                p15 = Puzzle15(cheats)
                p15.start()
                return f"4, {p15.moves}"
            else:
