        ...
        for (int j = 0; j < lock_count; j++){
            for (int k = 0; k < candidate_count; k++){
                if (locked[lost][k] == true){
                    lost = k;
// when lost is updated, remaining Ks are still iterating
//(ie locked[LOSTa][k=25] should trigger cycle = 1, but was prematurely changed to locked[LOSTb][k=25]), wrong pair of candidates used
// if a recursive function is used, can simply do check_func(int x = k) where func checks if locked[x][each candidate]==true
                    if (k == pairs[i].winner){
                        cycle = 1;
                    }
                }
