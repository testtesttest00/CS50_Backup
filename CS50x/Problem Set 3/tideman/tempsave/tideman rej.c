...
{
//    int x = 0;
//    if (pairs[pair_count-2].winner == pairs[pair_count-1].loser){
//        x = 1;
//    }
//    for (int i = 0; i < pair_count - x; i++){
    ...
    for (int i = 0; i < pair_count; i++){
        ...
        for(int k = 0; k < lock_count; k++){
            for(int j = 0; j < candidate_count; j++){
                if (locked[current_loser][j] == true){
                    current_loser = j;
                    if (pairs[i].winner == current_loser){
                        cycle = 1;
                        //printf("vvv cycle detected, loser %s vvv\n", candidates[current_loser]);
                        break;
                    }
                }
            }
