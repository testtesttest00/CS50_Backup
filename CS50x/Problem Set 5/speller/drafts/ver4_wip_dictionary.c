...
// pretty sure segmentation fault due to insufficient memory will continue using dictionaries/small from here on
            *(tableptr[index]) = n;
// different than the expression above: this can successfully update tableptr[index]
// (above expression derefences array, finds element at index, and saves results in a separate memory to assign to n->next)
// basically the top expression is at different temporary memory location than the bottom, but values are same
...
// (*tableptr) = node *table // node *table; table[i] = node_ptr_i; needs to dereference twice
                while(nodeptr != NULL){
