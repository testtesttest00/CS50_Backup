import sys
#make argv secret settings, easter egg eg cpu can cheat?
class TicTacToe():
    ...
    #editing dict(board)'s items does not trigger setter, writing in new dict in self.board triggers setter
    ...
    def nextmove(self):
        print(f"\n==-==-==-=-==-==-==-==-==-==-==-==-==-==-==-==")
        self.checkwin()
        if self.turn <= 9 and not self.lastmove: #lastmove flag for when win at < turn 9, creates excess loop
            ...
            self.checksetter()
            self.nextmove()
        elif self.turn > 9 and not self.lastmove: #lastmove flag for case for win at turn 9, triggers this excessively
    ...
    def checkwin(self):
        if self.board["a1"] == self.board["a2"] == self.board["a3"] or self.board["a1"] == self.board["b1"] == self.board["c1"] and not self.lastmove: #lastmove flag for when win with 2 intersecting lines, will trigger below lines excessively
            self.end(self.board["a1"])
        #elif changed to if otherwise case where x fills b column, o fills c column, no win triggered as a column filled with . is triggered, self.end(".") causes no result until a column is disrupted, b column wins
        if self.board["b1"] == self.board["b2"] == self.board["b3"] or self.board["a2"] == self.board["b2"] == self.board["c2"] and not self.lastmove:
            self.end(self.board["b2"])
        if self.board["c1"] == self.board["c2"] == self.board["c3"] or self.board["a3"] == self.board["b3"] == self.board["c3"] and not self.lastmove:
            self.end(self.board["c3"])
        if self.board["a1"] == self.board["b2"] == self.board["c3"] or self.board["a3"] == self.board["b2"] == self.board["c1"] and not self.lastmove:
            self.end(self.board["b2"])
