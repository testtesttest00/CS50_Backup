...
#                if str(pos) in winningpos:
#                    pos = f"\033[4m\033[1m{self.board[str(pos)]}\033[0m\033[0m"
#                    #print(pos)
#        return f"  A B C\n\
#1 {a1} {b1} {c1}\n\
#2 {a2} {b2} {c2}\n\
#3 {a3} {b3} {c3}\n"
        for pos in winningpos:
            self.board[pos] = f"\033[4m\033[1m{self.board[str(pos)]}\033[0m\033[0m"
        return f"  A B C\n\
...
3 {self.board["a3"]} {self.board["b3"]} {self.board["c3"]}\n"
    ...
    def checkwin(self, lastplaced = None): #lastplaced can be None as end will no longer be triggered at start of game
        ...
        #intersection wins will no longer trigger end twice, using elif instead of if

    def end(self, player = None, winningpos = None): #make strikethrough on winning postions and print?
        self.checksetter()
        if player in ["X", "O"]:
