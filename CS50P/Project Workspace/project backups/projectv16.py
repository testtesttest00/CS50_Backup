...
1 2 3 4 5 6 7"
    ...
    def board_input(self, player):#, coordinate): #coordinate is str: "x,y" x:1-7, y:1-6
        #x, y = int(coordinate.split(",")) #rmb that only choose column, piece falls to bottom
        ...
        for row in range(6):
            for column in self.board:
                ...
                if hori_x == 4:
                    ...
                    self.end("O")
        for row in range(3): #bottomleft-topright
            for column in range(4):
                #print(f"\n\nChecking Row:{row} Column:{column}\n")
                if self.board[column][row] == self.board[column+1][row+1] == self.board[column+2][row+2] == self.board[column+3][row+3] == "X":
                    ...
                    self.end("O")
        for row in range(6)[3:]: #topleft-bottomright
            for column in range(4):
                #print(f"\nChecking Row:{row} Column:{column}\n")
                if self.board[column][row] == self.board[column+1][row-1] == self.board[column+2][row-2] == self.board[column+3][row-3] == "X":
                    self.end("X")
...
#    board.setter
#    def board(self, board):
#        if len(board) == 6 and isinstance(board, list):
#            for column in board:
#                for value in column:
#                    if value in ["X", "O", "."]:
#                        pass
