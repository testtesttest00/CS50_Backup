    ...
    def winortie(self, current):
        if self.seq == "1":
            if current[4] != self.ai:
                ...
                if current[0] == ".":
                    return "a1"
                elif self.player == current[1]:
                    return "c1"
                elif self.player == current[3]:
                    return "a3"
            elif current[4] == self.ai:
                if self.player in [current[0], current[6]]:
                    return "b3"
                elif self.player in [current[2], current[8]]:
                    return "b1"
            else:
                pass
        #hard codes ai to take center if going first, although there are other winning methods
        elif self.seq == "2":
            if current[4] == self.player:
                return "a1"
            elif current[4] == self.player and current[0] == self.ai:
                pass
            #left 2 cases where player starts on edge or on corner to code for
            else:
                pass
